...
- (void)reachabilityChanged:(AFNetworkReachabilityStatus)status {
    switch (status) {
        ...
        case AFNetworkReachabilityStatusNotReachable:
            _netType = QKPublicNotReachable;
            NSLog(@"没有网络");
            break;
        case AFNetworkReachabilityStatusReachableViaWWAN:
            _netType = QKPublicWWAN;
            NSLog(@"正在使用3G网络");
            break;
        case AFNetworkReachabilityStatusReachableViaWiFi:
            _netType = QKPublicWiFi;
            NSLog(@"正在使用wifi网络");
            break;
    ...
    switch (httpType) {
        ...
        case QKPublicUPLOAD:
        {
            ...
            [_manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                ...
                if (params) {
                    [formData appendPartWithFileData:[params objectForKey:@"log.txt"] name:@"log.tex" fileName:@"txt" mimeType:@"txt"];
